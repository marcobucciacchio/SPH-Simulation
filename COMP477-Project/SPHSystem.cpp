...
SPHSystem::SPHSystem(unsigned int numParticles, float mass, float restDensity, float gasConst, float viscosity, float h, float g, float tension, float boxWidth) {
	...
	// kernel functions before the radius is known 
	DENSITY_KERN = 315.0f / (64.0f * PI * pow(h, 9));  // only first half of kernel func to calculate density minus (h^2 - r^2)^3
	PRESSURE_GRAD = -45.0f / (PI * pow(h, 6));  // only first half of kernel func for calculating pressure gradient 
	VISC_KERN = 45.0f / (PI * pow(h, 6));  // first half of the kernel func for calculating viscosity
	MASS = mass;
	GAS_CONSTANT = gasConst;
	H2 = h * h;
	SELF_DENS = MASS * DENSITY_KERN * pow(h, 6);
			...
			for (int y = -1; y <= 1; y++) {
				for (int z = -1; z <= 1; z++) {
					...
					while (pj != NULL) {
						...
						if (dist2 < H2 && pi != pj) {
							pDensity += MASS * DENSITY_KERN * glm::pow(H2 - dist2, 3); // finally completes the kernel function on which density effects the current particle
						}
						pj = pj->next;
					}
				}
				...
				for (int z = -1; z <= 1; z++) {
					...
					while (pj != NULL) {
						...
						if (dist2 < H2 && pi != pj) {
							...
							glm::vec3 pressureForce = -dir * MASS * (pi->pressure + pj->pressure) / (2 * pj->density) * PRESSURE_GRAD;
							pressureForce *= std::pow(h - dist, 2);
							pi->force += pressureForce;

							//apply viscosity force
							glm::vec3 velocityDif = pj->velocity - pi->velocity;
							glm::vec3 viscoForce = viscosity * MASS * (velocityDif / pj->density) * VISC_KERN * (h - dist);
							pi->force += viscoForce;
						}
						pj = pj->next;
					}
